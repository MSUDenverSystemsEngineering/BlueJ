...
Try {
	...
	If ($deploymentType -ine 'Uninstall' -and $deploymentType -ine 'Repair') {
		...
		## Uninstall Previous BlueJ installation
		Execute-MSI -Action Uninstall -Path '{8C92BA41-F0FF-4ECA-98D2-E6F0C013A4DB}'
		...
		$exitCode = Execute-MSI -Action "Install" -Path "$dirFiles\BlueJ-windows-422.msi" -Parameters "/qn" -PassThru
